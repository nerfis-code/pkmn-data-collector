...
import { downloadFile, getMoveWithFormat, getPkmnWithFormat, load, save } from './utils';

export async function getPokemonGroupByIdLimit(id: number) {
  const api = new MainClient();
  const pokemonGroup: Obj = {}
  const movsetGroup: Obj = {}
  for (let i = 1; i <= id; i++) {
    const pkmn = await api.pokemon.getPokemonById(i)
    ...
    movsetGroup[pkmn_data.name] = moveset
  }

  save(pokemonGroup, "pokemon")
  save(movsetGroup, "moveset")
